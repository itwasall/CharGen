...
def gen_starting_items(abilities, character_class=None):
    presence = abilities['Prescence']
    item_set_1 = [None, None, backpack, sack, small_wagon, "Donkey"]
    item_set_2 = [
        "Rope", f"{presence + 4} torches", f"Lanturn with oil for {presence + 6} hours", f"Magnesium Strip",
        f"{gen_random_scroll('unclean')}", "Sharp needle",
        f"Medicine chest - {presence + 4} uses (stops bleeding/infection and heals 1d6 health",
        "Metal file and lockpicks", "Bear trap (Presence DR14 to spot, 1d8 damage)",
    ...
    item_set_3 = [
        ...
        toolbox, "Heavy chain, 15 feet", "grappling hook",
    ...
    [regular_items.append(i) for i in [item_1, item_2, item_3, 'waterskin', f'{roll("1d4")} days of food'] if
     not None]
...
def gen_character(character_class=None, rand_class: bool = False, flag_list: List = []):
    ...
    for ability in character_abilities:
        ...
        ability_mod = 0
        if not character_class:
            if (ability_scores[sum(dice_rolls)] < 0) and (ability_re_rolls > 0):
                dice_rolls.reverse()
                dice_rolls.pop()
                dice_rolls.append(roll('1d6'))
                ability_re_rolls -= 1
        elif character_class not in ['Wretched Royalty'] and ability in list(
                optional_classes[character_class]['abilities'].keys()):
