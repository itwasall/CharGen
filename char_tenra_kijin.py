...
def gen_parts(mechanica_name: str, no_part_flag: bool = False) -> List:
    if not no_part_flag:
        return [part for part in mechanica_data['slots'] if mechanica_data['slots'][part]['slot'] == mechanica_name]
    else:
        return [part for part in mechanica_data['slots'] if ('no_slot' in mechanica_data['slots'][part]['slot'] and mechanica_name in mechanica_data['slots'][part]['replaces'])]
...
def gen_kijin(parts: List = None, verbose: bool = False):
    ...
    args:
        parts: List = Used to specify which mechanica parts you'll use. This is not choosing which mechanica you'll get,
            just the body parts the  character will be replacing. e.g. ['arm', 'torso']
        verbose: bool = There's some debug shit that's disabled by default
    """
    # Garbage seed shit
    seed(sum(ord(choice(printable)) for _ in range(1, 100)))
    # Removing 'homeopathic bullet skin' from part_names list. Needed earlier, not required now.
    part_names.pop()
    mechanica_parts: Tuple = ()
    part_class: List = []
    weapon_interface_guarantee: bool = False
    torso_guarantee: bool = False
    attempt_count = 0
    if not parts:
        ...
        # In order to make each mechanica part have interesting slots, if more than 3 slots are selected, a roll is
        #   made with a 2/3 chance of success that one of those mechanica parts is going to be the Weapon Interface
        if number_of_parts >= 3 and randint(1, 3) >= 2:
            weapon_interface_guarantee = True
        # As the torso has access to the Heart Engine, a mechanica part whose sole purpose is to power other
        #   mechanica parts, if more than 2 parts are selected there's a 1/2 chance that one of those parts is going to
        #   be the torso.
        if number_of_parts >= 2 and randint(1, 2) == 2:
            torso_guarantee = True
        while (len(mechanica_parts) <= 0
               # Prevents the situation where you only have the Weapon Interface. As it's job is to boost the power of
               #   other mechanica, it makes no sense to take this by itself.
               or ("weapon_interface" in mechanica_parts and len(mechanica_parts) <= 1)
               or (weapon_interface_guarantee and 'weapon_interface' not in mechanica_parts)
               or (torso_guarantee and 'torso' not in mechanica_parts)):
