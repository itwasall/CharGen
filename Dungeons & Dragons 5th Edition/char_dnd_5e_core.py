...
def multi(item, qty):
    return [item for _ in range(qty)]
"""
    ITEM PACKS
"""
BURGLURS_PACK = [BACKPACK, BALL_BEARINGS, BELL] + multi(CANDLE, 5) + [CROWBAR, HAMMER, LANTURN_HOODED, WATERSKIN, ROPE_HEMPEN] + multi(PITON, 10)
DIPLOMATS_PACK = [CHEST, CASE_MAP_SCROLL, CASE_MAP_SCROLL, CLOTHES_FINE, INK, INK_PEN, LAMP, FLASK, FLASK] + multi(PAPER, 5) + [PERFUME, SEALING_WAX, SOAP]
DUNGEONEERS_PACK = [BACKPACK, CROWBAR, HAMMER] + multi(PITON, 10) + multi(TORCH, 10) + multi(RATIONS, 10) + [TINDERBOX, WATERSKIN, ROPE_HEMPEN]
ENTERTAINERS_PACK = [BACKPACK, BEDROLL] + multi(CLOTHES_COSTUME, 2) + multi(CANDLE, 5) + multi(RATIONS, 5) + [WATERSKIN, DISGUISE_KIT]
EXPLORERS_PACK = [BACKPACK, BEDROLL, MESS_KIT, TINDERBOX] + multi(TORCH, 10) + multi(RATIONS, 10) + [WATERSKIN, ROPE_HEMPEN]
PRIESTS_PACK = [BACKPACK, BLANKET] + multi(CANDLE, 10) + [TINDERBOX, ALMS_BOX, CENSER, VESTMENTS, WATERSKIN, ROPE_HEMPEN] + multi(RATIONS, 2) + multi(BLOCK_OF_INCENSE, 2)
SCHOLARS_PACK = [BACKPACK, BOOK, INK, INK_PEN, SMALL_KNIFE, BAG_OF_SAND] + multi(PARCHMENT, 10)
