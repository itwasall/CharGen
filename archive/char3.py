...
with open('dnd_data\\classes.yaml') as f_classes:
    data_classes = yaml.safe_load(f_classes)
jobs = data_classes['classes']
backgrounds = yaml.safe_load(open   ('dnd_data\\backgrounds.yaml'))
equip = yaml.safe_load(open         ('dnd_data\\equipment.yaml'))
lang = yaml.safe_load(open          ('dnd_data\\languages.yaml'))
races = yaml.safe_load(open         ('dnd_data\\races.yaml'))
skills = yaml.safe_load(open        ('dnd_data\\skills.yaml'))
weapons = yaml.safe_load(open       ('dnd_data\\weapons.yaml'))
...
def job(race):
    ...
    job_weights = [w for w in race_job_weights[race]]
...
def alignment(race):
    ...
    character_alignment = str(choices(alignments_1, race['alignment_weights'][0])[0]) + str(choices(alignments_2, race['alignment_weights'][1])[0])
